...
This is the single source to feed into NotebookLM. It consolidates every page of the Central Casting demo into one document: the method, the orchestrator, the real catalogue, how the aimez.ai program runs on it, the cost demo about kill-and-respawn and long-run spend, and the v5 O0 rules in plain terms. It is written to read on its own and to drive a slide deck. The figure blocks describe visuals a generator can render. The slide outline near the end gives a deck structure.
...
- O4 UX and documentation: aimez site identity, the notes pages and the figures gallery.
- O5 patenting and authoring strategy: the StreetLight validation brief and provider disclosure boundaries.
...
- O1 authority and excavation, actors A1 and A2: the trace reconstruction lane, excavation lineage and chain execution behind the published figures.
- O2 research and lab, actors B1 and B2: the evidence closure lane and the figure-data chain.
- O16 publication, actors R16, P1 and P2: the exploratory findings manuscript chain, with R16 the research counterpart and P1, P2 the publication runners.
...
The durable truth lives in a folder called .cca, one lane per work area. The chat is disposable and the folder is the source of truth.
...
Figure 4. A horizontal flowchart: O0 chat slows, Fresh O0 chat loads the instruction layer, Reconcile from .cca, Resume routing. Caption: cycling the orchestrator costs one hydration turn.

## The v5 O0 rules, in plain terms
...
7. The .cca folder. Figure 2, the folder tree.
...
11. The real catalogue. The lane table and the four columns.
...
- O0: the central orchestrator, the zero element of the lane system. It plans, routes and reconciles, and it does not execute.
- Local orchestrator: a per-lane orchestrator, O1 through O16, that records step state for its lane and actors and relays verified summaries up to O0.
