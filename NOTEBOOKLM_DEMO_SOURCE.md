...
The durable truth lives in a folder called `.cc`, with one lane per work area. The chat is disposable and the folder is the source of truth.
...
Figure 4. A horizontal flowchart: 00 chat slows, then Fresh 00 chat loads the instruction layer, then Reconcile from .cc, then Resume routing. Caption: cycling the orchestrator costs one hydration turn.

## What the v5 00 rules mean, in plain terms
...
5. The .cc folder. Figure 2, the folder tree.
